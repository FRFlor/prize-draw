...
vi.useFakeTimers();
vi.mock('../classes/Timer.ts', () => ({
   getWaitTime: vi.fn().mockReturnValue(500)
}));
...
describe('RaffleBox', () => {
    ...
    beforeEach(async () => {
        ...
        wrapper = shallowMount(RaffleBox, {attachTo: document.body});
        await flushPromises();
    });
    ...
    describe('starts raffling when the raffle button is clicked', () => {
        ...
        it('stops raffling when there are no running applicants left', async () => {
            ...
            vi.advanceTimersByTime(A_LONG_LONG_TIME_PRETTY_MUCH_HOW_LONG_IT_TAKES_FOR_PHPSTORM_TO_INDEX);
            await flushPromises();
